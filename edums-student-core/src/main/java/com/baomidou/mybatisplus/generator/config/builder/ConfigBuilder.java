...
public class ConfigBuilder {
     ...
     * 模板路径配置信息
     */
    private final EdumsTemplateConfig template;
    /**
     * 数据库配置
     */
     ...
     */
    public ConfigBuilder(EdumsPackageConfig packageConfig, DataSourceConfig dataSourceConfig, StrategyConfig strategyConfig,
                         EdumsTemplateConfig template, GlobalConfig globalConfig) {
        // 全局配置
        ...
        // 模板配置
        if (null == template) {
            this.template = new EdumsTemplateConfig();
        } else {
            this.template = template;
        }
        // 包配置
        if (null == packageConfig) {
            handlerPackage(this.template, this.globalConfig.getOutputDir(), this.globalConfig.getResourceOutputDir(), new EdumsPackageConfig());
        } else {
            handlerPackage(this.template, this.globalConfig.getOutputDir(), this.globalConfig.getResourceOutputDir(), packageConfig);
        }
     ...
     * @return 所以模板路径配置信息
     */
    public EdumsTemplateConfig getTemplate() {
        return template == null ? new EdumsTemplateConfig() : template;
    }
     ...
     * @param outputDir
     * @param config    EdumsPackageConfig
     */
    private void handlerPackage(EdumsTemplateConfig template, String outputDir, String resourceOutputDir, EdumsPackageConfig config) {
        // 包信息
        ...
        packageInfo.put(Constants.FRONT_PAGE_PATH, joinPackage(config.getResourcesParent(), config.getViews()));
        packageInfo.put(Constants.FRONT_JS_PATH, joinPackage(config.getResourcesParent(), config.getJsPage()));
        ...
        if (null != configPathInfo) {
            ...
            setPathInfo(pathInfo, template.getListPage(), resourceOutputDir, Constants.FRONT_PAGE_PATH, Constants.FRONT_PAGE_PATH);
            setPathInfo(pathInfo, template.getFormPage(), resourceOutputDir, Constants.FRONT_PAGE_PATH, Constants.FRONT_PAGE_PATH);
            setPathInfo(pathInfo, template.getJsPage(), resourceOutputDir, Constants.FRONT_JS_PATH, Constants.FRONT_JS_PATH);
        }
    }
