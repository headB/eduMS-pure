...
import cn.wolfcode.edums.core.support.security.BASE64Encoder;
import cn.wolfcode.edums.core.support.security.Hex;
import cn.wolfcode.edums.core.support.security.coder.*;
...
public final class SecurityUtil {
    ...
    public static final String encryptMd5(String strSrc) {
        ...
        try {
            outString = encryptBASE64(AbstractMDCoder.encodeMD5(strSrc));
        } catch (Exception e) {
            throw new RuntimeException("加密错误，错误信息：", e);
        }
        return outString;
    ...
    public static final String md5(String strSrc) {
        String outString = null;
        try {
            outString = Hex.encodeHexString(AbstractMDCoder.encodeMD5(strSrc));
        } catch (Exception e) {
            throw new RuntimeException("加密错误，错误信息：", e);
        }
        return outString;
    ...
    public static final String decryptDes(String cryptData, byte[] key) {
        ...
        try {
            // 把字符串解码为字节数组，并解密
            decryptedData = new String(AbstractDESCoder.decrypt(decryptBASE64(cryptData), key));
        } catch (Exception e) {
            throw new RuntimeException("解密错误，错误信息：", e);
        }
    ...
    public static final String encryptDes(String data, byte[] key) {
        ...
        try {
            // 加密，并把字节数组编码成字符串
            encryptedData = encryptBASE64(AbstractDESCoder.encrypt(data.getBytes(), key));
        } catch (Exception e) {
            throw new RuntimeException("加密错误，错误信息：", e);
        }
    ...
    public static final String signRSA(String data, String privateKey) {
        try {
            return encryptBASE64(AbstractRSACoder.sign(data.getBytes(UTF8), decryptBASE64(privateKey)));
        } catch (Exception e) {
    ...
    public static final boolean verifyRSA(String data, String publicKey, String sign) {
        try {
            return AbstractRSACoder.verify(data.getBytes(UTF8), decryptBASE64(publicKey), decryptBASE64(sign));
        } catch (Exception e) {
    ...
    public static final String encryptRSAPrivate(String data, String privateKey) {
        try {
            return encryptBASE64(AbstractRSACoder.encryptByPrivateKey(data.getBytes(UTF8), decryptBASE64(privateKey)));
        } catch (Exception e) {
            throw new RuntimeException("解密错误，错误信息：", e);
        }
    }
    ...
    public static final String decryptRSAPublic(String cryptData, String publicKey) {
        try {
            // 把字符串解码为字节数组，并解密
            return new String(AbstractRSACoder.decryptByPublicKey(decryptBASE64(cryptData), decryptBASE64(publicKey)));
        } catch (Exception e) {
            throw new RuntimeException("解密错误，错误信息：", e);
        }
    }
    ...
    public static final String initHmacKey(String type) {
        try {
            return encryptBASE64(AbstractHmacCoder.initHmacKey(type));
        } catch (Exception e) {
    ...
    public static final String encryptHMAC(String type, String data, String key) {
        try {
            return AbstractHmacCoder.encodeHmacHex(type, data.getBytes(UTF8), decryptBASE64(key));
        } catch (Exception e) {
            throw new RuntimeException("加密错误，错误信息：", e);
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        System.out.println(SecurityUtil.encryptPassword("1"));
        ...
        Map<String, Object> key = AbstractRSACoder.initKey();
        String privateKey = encryptBASE64(AbstractRSACoder.getPrivateKey(key));
        String publicKey = encryptBASE64(AbstractRSACoder.getPublicKey(key));
