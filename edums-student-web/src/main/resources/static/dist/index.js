...
;layui.extend({setter: "config", admin: "lib/admin", view: "lib/view"}).define(["setter", "admin"], function (e) {
    var a = layui.setter, n = layui.element, i = layui.admin, t = i.tabsPage, l = layui.view, o = function () {
        ...
        if (layui.each(a.indPage, function (e, a) {
            if (s === a) return n = !0
        }), layui.config({base: a.base + "controller/"}), n || "/login" === s) r.render(t.path.join("/")).done(function () {
            i.pageType = "alone"
        }); else {
            if (a.interceptor) {
                var u = layui.data(a.tableName);
                if (!u[a.request.tokenName]) return location.hash = "/login/redirect=" + encodeURIComponent(s)
            }
